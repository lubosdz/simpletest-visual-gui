 ...
 * @author Bjoern Schotte <schotte at mayflower dot de>
 */
class WebDriver extends WebDriverNewApi
{
    /**
     * @var    string
     */
    ...
    /**
     * Session ID for new API
     */
    protected $sessionId;
    
    /**
     * Session ID for old API
     */
    protected $sessionIdOldApi;
    ...
    // errors returned from selenium server
    protected $error = array();
    
    // all returned responses from selenium server
    protected $log = array();
    
    // path to file [selenium.server.jar]
    protected $selenium_server_jar_path = false;

    protected $selenium_server_options = false;
    
    protected $selenium_capture_page_text = false;
    
    protected $selenium_write_directory = false;
...
Old API samples:
===============
[url] => http://localhost:4444/selenium-server/driver/?cmd=type&1=q&2=hello+world&sessionId=91ce8ebcf3a84ec5b86376fb7ed26fea
[command] => type
[response] => OK

[url] => http://localhost:4444/selenium-server/driver/?cmd=getNewBrowserSession&1=%2Afirefox&2=http%3A%2F%2Fwww.google.com
[command] => getNewBrowserSession
[response] => OK,91ce8ebcf3a84ec5b86376fb7ed26fea

[url] => http://localhost:4444/selenium-server/driver/?cmd=open&1=%2F&sessionId=91ce8ebcf3a84ec5b86376fb7ed26fea
[command] => open
[response] => OK

[url] => http://localhost:4444/selenium-server/driver/?cmd=waitForPageToLoad&1=10000&sessionId=91ce8ebcf3a84ec5b86376fb7ed26fea
[command] => waitForPageToLoad
[response] => Timed out after 10000ms

*/
     
    //public function __construct($browser, $browserUrl, $host = 'localhost', $port = 4444, $timeout = 30000)
    //public function __construct(WebDriverBase $parent, $element, $options){
    public function __construct($host = 'localhost', $port = 4444){
        ...
        */
        ...
        //$this->browserUrl = $this->getRequestURL();
        //$this->host = $parent->getHost();
        //$this->port = $parent->getPort();
        $this->timeout = WebDriverNewApi::CONNECT_TIMEOUT;
        
        // set path to selenium server (optional)
        if(defined('SELENIUM_SERVER_JAR_PATH')){
        	$this->selenium_server_jar_path = realpath(SELENIUM_SERVER_JAR_PATH);
        	// if invalid path, try to set working directory to where the file resides
        	if(!$this->selenium_server_jar_path){
	        	chdir(dirname(__FILE__));
    	    	$this->selenium_server_jar_path = realpath(SELENIUM_SERVER_JAR_PATH);
			}
		}
		
		if(defined('SELENIUM_WRITE_DIRECTORY')){
			$this->selenium_write_directory = realpath(SELENIUM_WRITE_DIRECTORY) . DIRECTORY_SEPARATOR;
        	if(!$this->selenium_server_jar_path){
	        	chdir(dirname(__FILE__));
    	    	$this->selenium_server_jar_path = realpath(SELENIUM_SERVER_JAR_PATH) . DIRECTORY_SEPARATOR;
			}
		}
		
		if(defined('SELENIUM_CAPTURE_PAGE_TEXT')){
			$this->selenium_capture_page_text = (bool) SELENIUM_CAPTURE_PAGE_TEXT;
		}
		
		if(defined('SELENIUM_SERVER_OPTIONS')){
			$this->selenium_server_options = (string) trim(SELENIUM_SERVER_OPTIONS);
		}

    }
    
    /**
    * Start java file running selenium server (normally selenium-server.jar)
    * Define path to the file via DEFINE('SELENIUM_SERVER_JAR_PATH', 'path-to->selenium-server.jar')
    */
    public function start_selenium_server(){

		// start selenium server via PHP - turned off
		return;

    	// we will attempt to start selenium server automatically from CLI (command line) command
    	if(file_exists($this->selenium_server_jar_path) && strtolower(pathinfo($this->selenium_server_jar_path, PATHINFO_EXTENSION)) == 'jar'){

    		// check that java is installed
    		$back = `PATH`;
    		if(strlen($back)){
    			// OK CLI work
				if(preg_match('/java/i', $back)){
					$message = 'OK - JAVA found installed on system.';
				}else{
					$message = 'CAUTION - JAVA not found installed on system.';
				}
				$this->addLog(array(
					'url' => '',
					'command' => 'CLI:PATH',
				...
				// we will attempt to start selenium server
				// however, we are not sure, whether we will succeed, 
				// so there is no way for PHP to determine success or failure, 
				// unless we call URL to selenium server, which will happen in calling getString()
				
				// CLI options:
				// ============
				// windows mode: [-multiwindow|singlewindow]
				// logging for server: [-log selenium.log]
				// logging for client: [-browserSideLog]
				
				
	    		$cmd = 'java -jar '.$this->selenium_server_jar_path;
	    		if($this->selenium_server_options != ''){
	    			$cmd .= ' '.$this->selenium_server_options;
				}
    			$back = `$cmd`;
			...
			exit('Cannot find path to selenium server (jar) file in ['.$this->selenium_server_jar_path.']');
		}
	}
    ...
    {
        $this->sessionIdOldApi = $this->getString("getNewBrowserSession", array('*'.$this->_browser, $this->browserUrl));
        return $this->sessionIdOldApi;
    }
     ...
     */
    public function setSpeedOldApi($value)
    {
     ...
     */
    public function getSpeedOldApi()
    {
     ...
     * @param string $windowID the JavaScript window ID of the window to select
     */
    public function selectWindowOldApi($windowID)
    {
     ...
     */
    public function getTitleOldApi()
    {
     ...
     */
    public function deleteCookieOldApi($name, $optionsString)
    {
...
<li>"default" - The default library.  Currently the default library is "ajaxslt" .</li>
    ...
    protected function doCommand($verb, $args = array())
    {
        ...
        if (isset($this->sessionIdOldApi)) {
            $url .= sprintf('&%s=%s', 'sessionId', $this->sessionIdOldApi);
        }

        // another possible option: cURL
        ...
        // shall we capture page text? (html not currently supported)
        if(false && $this->selenium_capture_page_text){
        	
        	if(!is_dir($this->selenium_write_directory)){
        		exit('Cannot capture page text. Writable directory does not exist in ['.$this->selenium_write_directory.']');
			}elseif(!is_writeable($this->selenium_write_directory)){
        		exit('Cannot capture page text. Directory is not writable in ['.$this->selenium_write_directory.']. Please adjust access permission for writing (e.g. 0777)');
			}
        	
        	$verb = 'getBodyText';
        	// "http://localhost:4444/selenium-server/driver/?cmd=getBodyText&sessionId=f645f7c3f5f347b58fd497a847b21b28"

	        $url = sprintf('http://%s:%s/selenium-server/driver/?cmd=%s', $this->host, $this->port, urlencode($verb));
	        
	        // capture session ID from last request
	        if(preg_match('@.*,([a-zA-Z0-9]+)@', $response, $sessionId)){

		        if (isset($sessionId[1])) {
		            $url .= sprintf('&%s=%s', 'sessionId', $sessionId[1]);
		        }

		        // another possible option: cURL
		        if (!$handle = fopen($url, 'r')) {
		            throw new Exception('Cannot connected to Selenium RC Server (while capturing body text)');
		        }
		        stream_set_blocking($handle, false);
		        $bodyText = stream_get_contents($handle);
		        fclose($handle);

		        // session-step-command
		        $fname = $sessionId[1].'_'.(count($this->log)-1).'_'.$verb.'.txt';
		        $path = $this->selenium_write_directory . $fname;
		        if(!file_put_contents($path, $bodyText)){
	        		exit('ERROR - Failed writing captured page text into ['.$path.']');
				}
			
			}else{
				$this->addLog('ERROR - Cannot capture page text because failed finding sessionID in response string ['.$response.']');
			}
		}
        
//        fclose($handle);

		// return original response from first request        
        return $response;
    }
    
    /**
    * Log message into array. Used to track responses from selenium server. Filters out error messages.
    * 
    * @param array|string $log Either Array('url' => ... , 'command' => ..., 'response' => ...) or any string $log to be logged.
    * @returns false if nothing has been logged, true otherwise
    */
    protected function addLog($log){
    	$cleanLog = array();

    	if(is_array($log)){
    		// we will create log from array
			$cleanLog = array(
	        	'url' => array_key_exists('url', $log) ? trim($log['url']) : '',
    	    	'command' => array_key_exists('command', $log) ? trim($log['command']) : '',
        		'response' => array_key_exists('response', $log) ? trim($log['response']) : ''
			);
		}elseif(trim($log)!=''){
			// we will create log from single string
			$cleanLog = array(
	        	'url' => $url,
    	    	'command' => $verb,
        		'response' => trim($log)
		...
		if(count($cleanLog)){
	        ...
	        // filter out errors
	        if(preg_match('/error|timeout|caution|timed out/i', $cleanLog['response'])){
        		$this->error[count($this->log)-1] = $cleanLog['response'];
			}
			return true;
		}

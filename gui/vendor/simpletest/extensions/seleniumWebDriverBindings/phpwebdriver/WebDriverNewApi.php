...
Definiciu prikazov pozri:
http://code.google.com/p/selenium/wiki/JsonWireProtocol
*/

class WebDriverNewApi extends WebDriverBase {
	...
	/**
	* Connection timeout for connection
	*/
    ...
    */
    public function connect($browserName="firefox", $version="", $caps=array()) {
        ...
        curl_setopt($session, CURLOPT_CONNECTTIMEOUT, self::CONNECT_TIMEOUT);
        $response = curl_exec($session);
        $err = curl_error($session);
        if(!$response || $err != ''){
        	throw new Exception('Failed connecting to Selenium server at ['.$request.'] - '.$err.'. Make sure Selenium server is running on target host server.');
		}
        $header = curl_getinfo($session);
        $this->requestURL = $header['url'];
        
        $this->_browser = $browserName;
        if(false !== stripos($header['url'], '/session/')){
        	list(, $this->sessionId) = explode('/session/', $header['url']);
		}
    }
    ...
    public function get($url) {
        ...
        /*
        // nevieme sharovat session medzi starym a novym selenium2 API
        // kazde API zacina session inym sposobom a ked zacneme pouzivat nove API, nie je mozne pouzit prikazy stareho API, pretoze nevieme sharovat session ID
    	$this->browserUrl = $url;
        if(empty($this->sessionIdOldApi)) {
        	$this->start();
		}
		*/
    }
    ...
    * @param int $msec milliseconds to wait until a single element is found
    */
    public function implicit_wait($msec) {
